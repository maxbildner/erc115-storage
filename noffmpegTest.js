...
function removeFiles(fileExtension) {
  fs.exists(
    `./video${fileExtension}`,
    (exists) => exists && fs.promises.unlink(`./video${fileExtension}`)
  );
  fs.exists(
    "./thumbnail.png",
    (exists) => exists && fs.promises.unlink("./thumbnail.png")
  );
}
...
async function uploadNFTMetadata(image, contentType, nftMetadata) {
  ...
  try {
    ...
    console.log(
      "http image URL: ",
      `https://nftstorage.link/ipfs/${metadata?.data?.image?.href?.slice(7)}`
    );
    ...
    console.log(
      "http video URL: ",
      `https://nftstorage.link/ipfs/${metadata?.data?.properties?.video?.href?.slice(
        7
      )}`
    );
...
async function main() {
  ...
  const base64Image = await fs.promises.readFile(filePath, {
    encoding: "base64",
  }); // string- base64
